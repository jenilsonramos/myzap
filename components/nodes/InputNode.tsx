...
const InputNode = ({ data, selected }: any) => {
    return (
        <div className={`w-80 group transition-all duration-300`}>
            <div className={`absolute -inset-[1px] bg-gradient-to-r from-orange-400 to-amber-400 rounded-[22px] blur opacity-0 transition-opacity duration-300 ${selected ? 'opacity-40' : 'group-hover:opacity-10'}`}></div>

            <div className={`relative bg-white/90 dark:bg-slate-950/80 backdrop-blur-xl rounded-[20px] shadow-sm border transition-all ${selected ? 'border-orange-400/50 shadow-[0_8px_30px_-6px_rgba(251,146,60,0.15)] ring-1 ring-orange-400/20' : 'border-slate-200 dark:border-white/5 hover:border-orange-300/30'}`}>
